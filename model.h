...
inline string tweettext(const json& tweet) {
    if (tweet["truncated"].is_boolean() && tweet["truncated"] == true) {
...
inline vector<string> splitwords(const string& text) {
    ...
    // added
    "rt", "retweet", "like", "just", "tomorrow", "new", "year", "month", "day", "today", "make", "let", "want", "did", "going", "good", "really", "know", "people", "got", "life", "need", "say", "doing", "great", "right", "time", "best", "happy", "stop", "think", "world", "watch", "gonna", "remember", "way",
    "better", "team", "check", "feel", "talk", "hurry", "look", "live", "home", "game", "run", "person", "house", "real", "thing", "lol", "has", "things", "thats", "fine", "said", "come", "coming", 
    "lets", "vs", "win", "says", "tell", "follow", "comes", "look", "looks", "post", "join", "add", "does", "went", "sure", "wait", "seen", "told", "yes", "video", "lot", "looks", "long",
    "im", "youre", "thats", "ive", "youve", "yall", "didnt", "havent", "wont", "cant", "dont", "shouldnt", "hasnt", "doesnt", "id", "its", "whats", "were", "youll", "lets", "yall",
    // "e280a6", "\xe2\x80\xa6",
    ...
    // exclude entities, urls and some contractions from this words list

    // "(`)|(\xE2\xB3\xBF)|(\xEF\xBC\x87)|(\xC2\xB4)|(\xD7\xB3)|(\xE2\x80\xB2)|(\xE2\x80\xB5)|(\xe2\x80\xa6)|"

    static const regex ignore(R"((^[^[:print:]]+$)|(n?[^\w\d\\\]\[+_)(*&^%$#@!~|{}":?></.,;]+[smdvnralt][\w]*$)|(\xe2\x80\xa6)|(&[\w]+;)|((http|ftp|https)://[\w-]+(.[\w-]+)+([\w.,@?^=%&:/~+#-]*[\w@?^=%&/~+#-])?))");
...
struct TimeRange
{
    ...
    TimeRange() {}
    TimeRange(timestamp b, timestamp e) : begin(b), end(e) {}
...
{
    User() {}
    explicit User(const json& user, milliseconds t) 
        : data(make_shared<shared>(shared{user, t})) 
    {}
    struct shared 
    {
        shared() {}
        explicit shared(const json& u, milliseconds t) 
            : user(u)
            , active(t, t)
        {}
        json user;
    ...
    void tweet(milliseconds t) {
        auto u = make_shared<shared>(*data);
        ++u->tweets;
        u->active.end = t;
        data = u;
    }
    void retweet(milliseconds t) {
        auto u = make_shared<shared>(*data);
        ++u->retweets;
        u->active.end = t;
        data = u;
    }
    void quote(milliseconds t) {
        auto u = make_shared<shared>(*data);
        ++u->quotetweets;
        u->active.end = t;
        data = u;
    ...
    friend bool operator<(const User& l, const User& r){
        return l.data->user["id_str"].get<string>() < r.data->user["id_str"].get<string>();
    }
    friend bool operator==(const User& l, const User& r){
        return l.data->user["id_str"].get<string>() == r.data->user["id_str"].get<string>();
    }
...
{
    UserCount() {}
    explicit UserCount(User u) : user(u) {}
    User user;
    int count = 0;
    vector<float> all;
};
...
struct ViewModel
{
    ...
    explicit ViewModel(Model& m) : m(m) {
        ...
        if (scope == scope_selected && idx >= 0 && idx < int(model.groups.size())) {
            ...
            data->words |=
                ranges::action::stable_sort([](const WordCount& l, const WordCount& r){
                    return l.count > r.count;
                });

            data->users = group->users |
                ranges::view::transform([&](const pair<string, User>& user){
                    return UserCount{user.second};
                });

            data->users |=
                ranges::action::stable_sort([](const UserCount& l, const UserCount& r){
                    return l.user.data->retweets + l.user.data->tweets + l.user.data->quotetweets > 
                        r.user.data->retweets + r.user.data->tweets + r.user.data->quotetweets;
                });
        } else {
            ...
            data->scope_users = &data->allusers;
...

        {
            ...
            groups |=
                ranges::action::stable_sort([](const pair<milliseconds, float>& l, const pair<milliseconds, float>& r){
                    return l.first < r.first;
                });
    ...
    struct shared
    {
        ...
        vector<UserCount> users;
        vector<UserCount> allusers;
        ...
        const vector<UserCount>* scope_users = nullptr;
